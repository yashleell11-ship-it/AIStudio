"""Elf Toon online source connector (Themesia MangaReader HTML)."""
...
class ElfToonConnector(SourceConnector):
    """Browse and read manhwa/manhua from Elf Toon (elftoon.com)."""

    SOURCE_TYPE = "elftoon"
    DISPLAY_NAME = "Elf Toon"
    DESCRIPTION = (
        "Browse and read manhwa and manhua from Elf Toon. "
        "Images are proxied through ManhwaManiacs. "
        "Coin-locked chapters on the source are omitted."
    )
    BROWSABLE = True
    ...
    def allowed_image_hosts(self) -> frozenset[str]:
        # Covers often via Jetpack CDN (iN.wp.com); pages on elftoon.com.
        return frozenset({"elftoon.com", "wp.com"})
    ...
    def _log(
        ...
        *,
        ...
        message = (
            f"Elf Toon {operation} {SITE_BASE}{path} "
    ...
    def search_series(self, query: str, page: int, *, sort: str | None = None) -> PaginatedSeriesList:
        del sort  # site search ignores browse order
        if page < 1:
            page = 1
    ...
    def get_chapter_pages(self, chapter_id: str) -> list[Page]:
        ...
        if path is None:
            # Accept bare ``{series}-chapter-N`` ids
