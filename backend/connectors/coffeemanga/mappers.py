"""Map CoffeeManga (Madara theme) HTML to normalized connector models.

CoffeeManga (coffeemanga.ink) runs the same WordPress "Madara" theme as
Toonily, so the shape of this module mirrors ``connectors/toonily/mappers.py``.
The site-specific deviations captured from the live markup are:

- series/chapter URLs live under ``/manga/`` (Toonily uses ``/serie/``);
- browse listing lives at ``/manga/`` (Toonily uses ``/webtoons/``);
- search results are rendered in ``div.row.c-tabs-item__content`` blocks that
  ALSO contain per-series chapter links, so search must be scoped to that
  container instead of scraping every ``/manga/`` href;
- the detail ``<h1>`` is preceded by a ``manga-title-badges`` span and the
  ``<h5>`` metadata labels carry surrounding whitespace, so those regexes are
  looser than Toonily's; the author label is ``Author(s)`` not ``Writer(s)``;
- reader image URLs carry a LEADING SPACE and the lazy images keep the real
  URL in ``data-src`` (``src`` is a data-URI placeholder), so page parsing is
  done per ``<img>`` tag with an explicit ``.strip()`` — a naive clone of
  Toonily's alternation silently drops the eager pages.
"""
...
SITE_BASE = "https://coffeemanga.ink"
PAGE_SIZE = 20

# Browse (page-item-detail) and search (c-tabs-item__content) listings are
# parsed by splitting the page into per-card segments and parsing each card in
# isolation. Splitting (rather than one big `.*?` regex) stops a coverless card
# from borrowing the next card's image and lets the cover resolve to data-src
# (lazy thumbnails) instead of a data: placeholder.
...
# The first /manga/{slug}/ anchor carrying a title is the series link; ([^"/]+)
# keeps it from matching the block's chapter links (…/{slug}/chapter-N/).
...
# Reader page images: match each <img> carrying the wp-manga-chapter-img class,
# then pull the real URL out of the tag (see _extract_image_url).
CHAPTER_IMG_TAG_RE = re.compile(r"<img\b[^>]*wp-manga-chapter-img[^>]*>", re.I)
_IMG_DATA_SRC_RE = re.compile(r'data-src="\s*([^"]+?)\s*"', re.I)
# (?<!-) keeps this from matching data-src; src="..." (not srcset) only.
...
def _extract_image_url(tag: str) -> str | None:
    """Real image URL from an ``<img>`` tag.

    Prefer ``data-src`` (lazy images keep the real URL there while ``src`` is a
    data-URI placeholder), fall back to ``src`` (eager images); skip ``data:``
    placeholders and strip the leading space this site emits on image URLs.
    """
    for pattern in (_IMG_DATA_SRC_RE, _IMG_SRC_RE):
...
def _parse_cards(html_text: str, split_re: re.Pattern[str], marker: str) -> list[Series]:
    ...
    for segment in split_re.split(html_text):
        # Skip the pre-first-card preamble; only real card segments lead with
...
def parse_search_results(
    ...
    *,
    ...
    if not items and query.strip():
        # Fallback: some search layouts omit the tabbed container; take unique
        # series slugs (no chapter suffix) so a result set is still returned.
        seen: set[str] = set()
...
def parse_series_detail(html_text: str, series_id: str) -> Series | None:
    # The <h1> is preceded by a manga-title-badges span; skip it with .*? and
    # capture only the leading text node ([^<] excludes any badge/alt-title
    # span nested inside the heading), falling back to og:title.
