"""CoffeeManga online source connector."""
...
from connectors.coffeemanga.mappers import (
    PAGE_SIZE,
    SITE_BASE,
    chapter_id_to_path,
    listing_params,
...
class CoffeeMangaConnector(SourceConnector):
    """Browse and read manga/manhwa from CoffeeManga (Madara HTML catalog)."""

    SOURCE_TYPE = "coffeemanga"
    DISPLAY_NAME = "CoffeeManga"
    DESCRIPTION = (
        "Browse and read manga and manhwa from CoffeeManga. "
        "Images are proxied through ManhwaManiacs for reliable local reading."
    )
    BROWSABLE = True
    SUPPORTS_IMPORT = False
    # CoffeeManga is a general catalogue (like MangaDex/AsuraScans), not a
    # dedicated adult site, so it is not gated behind the mature toggle;
    # individual adult series are handled by the per-series content filter.
    MATURE = False

    def __init__(self) -> None:
        # CoffeeManga sits behind Cloudflare; use browser impersonation exactly
        # as the Toonily connector does (proven against the same protection).
    ...
    def _log_request(
        ...
        *,
        ...
        message = (
            f"CoffeeManga {operation} {SITE_BASE}{path} "
    ...
    def _remember_page_count(self, chapter_id: str, page_count: int) -> None:
        ...
        logger.info(
            "CoffeeManga cached page_count chapter_id=%s count=%d",
            chapter_id,
            page_count,
        )
