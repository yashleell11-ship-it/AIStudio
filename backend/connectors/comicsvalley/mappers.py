"""Map ComicsValley (Madara) HTML to normalized connector models.

ComicsValley.com is behind a Cloudflare JS challenge. The live catalog is on
``comicsvalley.net`` (Madara ``/manga/``). Series pages have empty chapter
lists and point "Read Online" at ``allporncomics.co/comic/{slug}/``, which
hosts the actual Madara chapter/reader content. This mapper therefore:

- browse/search/detail → comicsvalley.net
- chapters/pages → allporncomics.co (``/comic/`` segment)
"""
...
SITE_BASE = "https://comicsvalley.net"
READER_BASE = "https://allporncomics.co"
PAGE_SIZE = 20
...
# Reader host uses /comic/{series}/{chapter}/
CHAPTER_LINK_RE = re.compile(
    r'<li class="wp-manga-chapter[^"]*">\s*'
    r'<a[^>]+href="(?:https?://[^"]+)?/comic/([^"]+)/"[^>]*>\s*([^<]+)</a>',
    re.S | re.I,
)
...
READ_ONLINE_RE = re.compile(
    r'href="(https?://(?:www\.)?allporncomics\.co/comic/[^"]+/)"',
    re.I,
)
...
def parse_read_online_url(html_text: str, series_id: str) -> str:
    """Return the AllPornComics series URL from a ComicsValley detail page."""
    match = READ_ONLINE_RE.search(html_text)
    if match:
        return match.group(1).rstrip("/") + "/"
    return f"{READER_BASE}{reader_series_path(series_id)}"


def parse_manga_id(html_text: str) -> str | None:
    match = re.search(r"\bpostid-(\d+)\b", html_text, re.I)
...
def _parse_chapter_segment_parts(segment: str) -> tuple[int, int, int] | None:
    ...
    # APC-style segments: the-recruit-1, valentinas-choice-2
...
def absolute_reader_url(path: str) -> str:
    """Build an absolute allporncomics.co URL from a /comic/... path."""
    if path.startswith("http://") or path.startswith("https://"):
        return path
    return f"{READER_BASE.rstrip('/')}/{path.lstrip('/')}"
