"""3Hentai online source connector (custom Laravel catalog)."""
...
from connectors.threehentai.mappers import (
...
class ThreeHentaiConnector(SourceConnector):
    """Browse and read galleries from 3Hentai (nhentai-style HTML catalog)."""

    SOURCE_TYPE = "3hentai"
    DISPLAY_NAME = "3Hentai"
    DESCRIPTION = (
        "Browse and read hentai doujinshi and manga from 3Hentai. "
    ...
    def get_series_list(self, page: int, *, sort: str | None = None) -> PaginatedSeriesList:
        if page < 1:
            page = 1
        if sort == "english":
            listing = self._fetch_listing(page, query=ENGLISH_LANGUAGE_QUERY)
        elif sort == "popular":
            listing = self._fetch_listing(page, sort="popular")
        else:
            listing = self._fetch_listing(page)
        logger.info(
            "3Hentai browse sort=%r page=%d count=%d has_more=%s",
            sort,
            page,
            len(listing.items),
            listing.has_more,
    ...
    def search_series(self, query: str, page: int, *, sort: str | None = None) -> PaginatedSeriesList:
        ...
        if sort == "english":
            ...
            listing = self._fetch_listing(page, query=combined, sort=sort if sort == "popular" else None)
        elif not normalized:
            ...
            listing = self._fetch_listing(
                page,
                query=normalized,
                sort=sort if sort == "popular" else None,
            )
        logger.info(
            "3Hentai search page=%d count=%d sort=%r query=%r",
            page,
            len(listing.items),
            sort,
            normalized,
